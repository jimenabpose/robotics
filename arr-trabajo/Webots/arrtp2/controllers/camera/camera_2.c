...
#define SPEED 40
#define TIME_STEP 64
#define COLOR_BLOB_VALUE 700000
  ...
  while(wb_robot_step(TIME_STEP)!=-1) {
    ...
    if (pause_counter == 0) {
      ...
      if (blue >= COLOR_BLOB_VALUE) {
        ...
        pause_counter = 20;
        wb_camera_save_image(camera,"green_blob.png",100); // take a snapshot of it!
      } else if (red >= COLOR_BLOB_VALUE) {
        printf("Looks like I found a red blob !\n");
        pause_counter = 20;
        wb_camera_save_image(camera,"red_blob.jpg",100); // take a snapshot of it!
