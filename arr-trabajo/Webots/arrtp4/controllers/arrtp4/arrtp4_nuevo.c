...
int getReward(int newState){
	...
	int nearToBarState = 3;
	int pushingForwardState = 1;
	
	if(s0 != nearToBarState || s7 != nearToBarState) {
		return -1;
	}
	
	if(s1 != pushingForwardState || s6 != pushingForwardState) {
		return 0;
	}
  ...
  if(sensors_value[0] < minDistance || sensors_value[7] < minDistance) {
  		return -1;
	}
  
  if(sensors_value[0] - sensors_value[7] > maxDifference 
  		|| sensors_value[0] - sensors_value[7] < -maxDifference) {
  	return 0;
  } 
  
  if(sensors_value[1] - sensors_value[6] > maxDifference 
  		|| sensors_value[1] - sensors_value[6] < -maxDifference) {
  	return 0;
  }
...
void performAtion(int actionNumber) {
	...
	if (actionNumber < 3) {
		...
		int i;
		int forwardSpeed = 400;
		if(actionNumber == 3 || actionNumber == 1) {
		...
		} else if(actionNumber == 4 || actionNumber == 2) {
			speed[0] = 300;
			speed[1] = -300;
